import torch
import torch.nn as nn

#CAE整体模型
class CAE(nn.Module):
    def __init__(self):
        super().__init__()
        #CAE模型包含一个Encoder编码器，一个Decoder解码器，这两个模型在下面实现
        self.encoder = Encoder()
        self.decoder = Decoder()

    def forward(self, x):
        #CAE的整个流程，先把x用编码器编码，再将编码的结果用解码器解码
        return self.decoder(self.encoder(x))

    #用于在过程中保存模型，将encoder和decoder分别保存
    def save_model(self, exp_name, epoch_idx):
        torch.save(self.encoder.state_dict(), f"../experiments/{exp_name}/chkpt/encoder_{epoch_idx}.pth")
        torch.save(self.decoder.state_dict(), f"../experiments/{exp_name}/chkpt/decoder_{epoch_idx}.pth")

    #用于在训练完成时保存模型，同上，这一个函数基本没用到
    def save_final_model(self, exp_name):
        torch.save(self.encoder.state_dict(), f"../experiments/{exp_name}/chkpt/encoder_final.pth")
        torch.save(self.decoder.state_dict(), f"../experiments/{exp_name}/chkpt/decoder_final.pth")

    #导入模型，用于test
    def load_model(self, encode, decode):
        self.encoder.load_state_dict(torch.load(encode))
        self.decoder.load_state_dict(torch.load(decode))

    #CAE.encode函数，调用CAE的encoder进行编码，下面的decode同理
    #这两步只是换个名，为了在train和test的代码中看起来好看一些
    def encode(self,x):
        return self.encoder(x)

    def decode(self,x):
        return self.decoder(x)

#Encoder编码器模型
class Encoder(nn.Module):
    def __init__(self):
        super().__init__()
        # 下面的都是网络模型，前面的注释是输出的尺寸
        # 注意最后一层是全连接层，其他的都是卷积层
        # 64x64x64
        self.e_conv_1 = nn.Sequential(
            nn.ZeroPad2d((1, 2, 1, 2)),
            nn.Conv2d(in_channels=3, out_channels=64, kernel_size=(5, 5), stride=(2, 2)),
            nn.LeakyReLU()
        )
        # 128x32x32
        self.e_conv_2 = nn.Sequential(
            nn.ZeroPad2d((1, 2, 1, 2)),
            nn.Conv2d(in_channels=64, out_channels=128, kernel_size=(5, 5), stride=(2, 2)),
            nn.LeakyReLU()
        )
        # 128x32x32
        self.e_block_1 = nn.Sequential(
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
            nn.LeakyReLU(),
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
        )
        # 128x32x32
        self.e_block_2 = nn.Sequential(
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
            nn.LeakyReLU(),
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
        )
        # 128x32x32
        self.e_block_3 = nn.Sequential(
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
            nn.LeakyReLU(),
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
        )
        # 32x32x32
        self.e_conv_3 = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=32, kernel_size=(5, 5), stride=(1, 1), padding=(2, 2)),
            nn.Tanh()
        )
        # fully connect 512
        self.fc_512 = nn.Sequential(
            nn.Linear(in_features=32 * 32 * 32, out_features=1024),
            nn.Tanh(),
            nn.Linear(in_features=1024, out_features=512),
            nn.Tanh()
        )

    def forward(self, x):
        #下面的内容是编码器的执行顺序
        #例如输入图片x，先把x用e_conv_1卷积，得到ec1，以此类推
        ec1 = self.e_conv_1(x)
        ec2 = self.e_conv_2(ec1)
        eblock1 = self.e_block_1(ec2) + ec2
        eblock2 = self.e_block_2(eblock1) + eblock1
        eblock3 = self.e_block_3(eblock2) + eblock2
        ec3 = self.e_conv_3(eblock3)  # in [-1, 1] from tanh activation

        #ec3是最后一层卷积层，输出的尺寸是(batch_size,32,32,32)
        #这一句代码的目的是获取batch_size
        batch_size = ec3.shape[0]

        #在这一步修改ec3的尺寸，将其尺寸改为(batch_size,32*32*32)
        #相当于把后面的32x32x32拍扁成一维向量，目的是输入后面的全连接层
        #fc512全连接层的输出结果是(batch_size,512)
        fc512 = self.fc_512(ec3.reshape(shape=(batch_size, 32 * 32 * 32)))

        #这一步是将全连接层的输出结果二值化
        #全连接层的输出结果是-1~1的浮点向量，在这一步二值化为0|1向量
        with torch.no_grad():
            rand = torch.rand(fc512.shape).cuda()
            prob = (1 + fc512) / 2
            eps = torch.zeros(fc512.shape).cuda()
            eps[rand <= prob] = (1 - fc512)[rand <= prob]
            eps[rand > prob] = (-fc512 - 1)[rand > prob]

        # encoded tensor
        encoded = 0.5 * (fc512 + eps + 1)  # (-1|1) -> (0|1)
        #encoded即为编码后的结果
        #encoded的尺寸为(batch_size,512)

        return encoded

#Decoder解码器
class Decoder(nn.Module):
    def __init__(self):
        super().__init__()
        #下面是解码器的结构，与Encoder相似
        #但是需要注意在d_fc512里，先将512维向量通过全连接层修改为32*32*32维
        #再进行resize，变成(batch_size,32,32,32)
        # 32x32x32
        self.d_fc512 = nn.Sequential(
            # 512 -> 1024
            nn.Linear(in_features=512, out_features=1024),
            nn.Tanh(),
            # 1024 -> 32x32x32
            nn.Linear(in_features=1024, out_features=32 * 32 * 32),
            nn.Tanh()
        )
        # 128x64x64
        self.d_up_conv_1 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(3, 3), stride=(1, 1)),
            nn.LeakyReLU(),
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.ConvTranspose2d(in_channels=64, out_channels=128, kernel_size=(2, 2), stride=(2, 2))
        )
        # 128x64x64
        self.d_block_1 = nn.Sequential(
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
            nn.LeakyReLU(),
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
        )
        # 128x64x64
        self.d_block_2 = nn.Sequential(
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
            nn.LeakyReLU(),
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
        )
        # 128x64x64
        self.d_block_3 = nn.Sequential(
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
            nn.LeakyReLU(),
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.Conv2d(in_channels=128, out_channels=128, kernel_size=(3, 3), stride=(1, 1)),
        )
        # 256x128x128
        self.d_up_conv_2 = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=32, kernel_size=(3, 3), stride=(1, 1)),
            nn.LeakyReLU(),
            nn.ZeroPad2d((1, 1, 1, 1)),
            nn.ConvTranspose2d(in_channels=32, out_channels=256, kernel_size=(2, 2), stride=(2, 2))
        )
        # 3x128x128
        self.d_up_conv_3 = nn.Sequential(
            nn.Conv2d(in_channels=256, out_channels=16, kernel_size=(3, 3), stride=(1, 1)),
            nn.LeakyReLU(),
            nn.ReflectionPad2d((2, 2, 2, 2)),
            nn.Conv2d(in_channels=16, out_channels=3, kernel_size=(3, 3), stride=(1, 1)),
            nn.Tanh()
        )

    def forward(self, x):
        y = x * 2.0 - 1  # (0|1) -> (-1|1)

        #这里获取batch_size,并将(batch_size,32*32*32)转换为(batch_size,32,32,32)
        batch_size = y.shape[0]
        d32x32x32 = self.d_fc512(y).reshape(shape=(batch_size, 32, 32, 32))

        uc1 = self.d_up_conv_1(d32x32x32)
        dblock1 = self.d_block_1(uc1) + uc1
        dblock2 = self.d_block_2(dblock1) + dblock1
        dblock3 = self.d_block_3(dblock2) + dblock2
        uc2 = self.d_up_conv_2(dblock3)
        decoded = self.d_up_conv_3(uc2)

        return decoded
