...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        fc256 = self.fc_256(ec3.reshape(shape=(batch_size, 32 * 32 * 32)))

        # Binarisation
        with torch.no_grad():
            rand = torch.rand(fc256.shape).cuda()
            prob = (1 + fc256) / 2
            eps = torch.zeros(fc256.shape).cuda()
            eps[rand <= prob] = (1 - fc256)[rand <= prob]
            eps[rand > prob] = (-fc256 - 1)[rand > prob]

        encoded = 0.5 * (fc256 + eps + 1)  # (-1|1) -> (0|1)
        return encoded
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        d32x32x32 = self.d_fc256(y).reshape(shape=(batch_size, 32, 32, 32))
